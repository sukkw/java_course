			...
			try {
				socket.connect(
						new InetSocketAddress(InetAddress
								.getByName("localhost"), port), 15000);
				...
				MessageDialog.showMessage(ui, "Unknown host!", 1);
				socket = null;
				break;
				...
				MessageDialog.showMessage(ui, "Connection timeout. Try again later!", 1);
				break;
			} catch (IOException e1) {
				socket = new Socket();
				port++;
				if (port == 7005) {
					MessageDialog.showMessage(ui, " Can't find chat server! Maybe its not started!",
							1);
					socket = null;
					break;
				}
			...
			try {
				...
				if (msg.type.equals("message")) {
					if (msg.recipient.equals(ui.username)) {
						ui.showMessage("[" + msg.sender + " > Me] : "
								+ msg.content + "\n");
					} else {
						ui.showMessage("[" + msg.sender + " > " + msg.recipient
								+ "] : " + msg.content + "\n");
					}
				}

				else if (msg.type.equals("newuserz")) {

					if (!msg.content.equals(ui.username)) {
						ui.showMessage("[SERVER] : User " + msg.content
								+ " connected");
					}
					...
					if (msg.content.equals("TRUE")) {
						...
						MessageDialog.showMessage(ui, msg.content, 1);
					}
					...
					MessageDialog.showMessage(ui, "disconnected", 1);
					socket.close();
					ui.dispose();
				}
			} catch (Exception ex) {
